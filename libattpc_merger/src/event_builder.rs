...
/// EventBuilder takes GrawFrames and composes them into GetEvents.
///
/// The EventBuilder recieves data from the Merger and constructs an Event struct. The
/// GetEvent struct can then be sent to an HDFWriter to write merged events to disk.
#[derive(Debug)]
...
impl EventBuilder {
    ...
    /// Returns a `Result<Option<GetEvent>>`. If the Option is None, the event being built is not complete. If the Optiion is Some,
    /// the event being built was completed, and a new event was started for the frame that was passed in.
    #[allow(clippy::comparison_chain)]
    pub fn append_frame(
        &mut self,
        frame: GrawFrame,
    ) -> Result<Option<GetEvent>, EventBuilderError> {
        if let Some(current_id) = self.current_event_id {
            if frame.header.event_id < current_id {
                ...
                let event = GetEvent::new(&self.pad_map, &self.frame_stack)?;
